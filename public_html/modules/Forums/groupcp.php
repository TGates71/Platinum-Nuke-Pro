...
/*****************************************************/
function generate_user_info(&$row, $date_format, $group_mod, &$from, &$posts, &$joined, &$poster_avatar, &$profile_img, &$profile, &$search_img, &$search, &$pm_img, &$pm, &$email_img, &$email, &$www_img, &$www, &$icq_status_img, &$icq_img, &$icq, &$aim_img, &$aim, &$msn_img, &$msn, &$yim_img, &$yim, &$userdata, &$online_status)
{
...
/*****************************************************/
	if ( $row['user_session_time'] >= (time()-60) )
	{
		...
		{
			$online_status = '<strong><i><a href="' . append_sid("viewonline.$phpEx") . '" title="' . sprintf($lang['is_hidden'], $row['username']) . '"' . $hidden_color . '>' . $lang['Hidden'] . '</a></i></strong>';
		}
		else
		{
			$online_status = '<strong><span title="' . sprintf($lang['is_offline'], $row['username']) . '"' . $offline_color . '>' . $lang['Offline'] . '</span></strong>';
		}
	}
	else
	{
...
if ((isset($_POST['move_user']) || isset($_POST['copy_user'])) && group_id)
{
	...
	if ( isset($_GET['new_g']) || isset($_POST['new_g']) )
	{
		$new_group_id = ( isset($_POST['new_g']) ) ? intval($_POST['new_g']) : intval($_GET['new_g']);
	}
	else
	{
		$new_group_id = 0;
...
/* Forum - User Online Status v.1.0.5          START */
/*****************************************************/
        $sql = "SELECT username, user_color_gc, user_id, user_viewemail, user_posts, user_regdate, user_from, user_website, user_email, user_icq, user_aim, user_yim, user_msnm, user_allow_viewonline, user_session_time
                FROM " . USERS_TABLE . "
...
/* Forum - User Online Status v.1.0.5          START */
/*****************************************************/
        $sql = "SELECT u.username, u.user_color_gc, u.user_id, u.user_viewemail, u.user_posts, u.user_regdate, u.user_from, u.user_website, u.user_email, u.user_icq, u.user_aim, u.user_yim, u.user_msnm, ug.user_pending, u.user_allow_viewonline, u.user_session_time
...
/* Forum - User Online Status v.1.0.5          START */
/*****************************************************/
        generate_user_info($group_moderator, $board_config['default_dateformat'], $is_moderator, $from, $posts, $joined, $poster_avatar, $profile_img, $profile, $search_img, $search, $pm_img, $pm, $email_img, $email, $www_img, $www, $icq_status_img, $icq_img, $icq, $aim_img, $aim, $msn_img, $msn, $yim_img, $yim, $userdata, $online_status);
/*****************************************************/
/* Forum - User Online Status v.1.0.5            END */
/*****************************************************/
...
/* Forum - User Online Status v.1.0.5          START */
/*****************************************************/
                generate_user_info($group_members[$i], $board_config['default_dateformat'], $is_moderator, $from, $posts, $joined, $poster_avatar, $profile_img, $profile, $search_img, $search, $pm_img, $pm, $email_img, $email, $www_img, $www, $icq_status_img, $icq_img, $icq, $aim_img, $aim, $msn_img, $msn, $yim_img, $yim, $userdata, $online_status);
/*****************************************************/
/* Forum - User Online Status v.1.0.5            END */
/*****************************************************/
...
/* Forum - User Online Status v.1.0.5          START */
/*****************************************************/
                                generate_user_info($modgroup_pending_list[$i], $board_config['default_dateformat'], $is_moderator, $from, $posts, $joined, $poster_avatar, $profile_img, $profile, $search_img, $search, $pm_img, $pm, $email_img, $email, $www_img, $www, $icq_status_img, $icq_img, $icq, $aim_img, $aim, $msn_img, $msn, $yim_img, $yim, $userdata, $online_status);
/*****************************************************/
/* Forum - User Online Status v.1.0.5            END */
/*****************************************************/
...
else
{
        ...
        $in_group = array();
//----------------------------------------------------------------------
// Enhanced Group List - Begin Code Alteration
//
	$group_list_out = $member_groups = $pending_groups = array();
//
// Enhanced Group List - End Code Alteration
//----------------------------------------------------------------------
        if ( $userdata['session_logged_in'] )
        {
/*----------------------------------------------------------------------
// Enhanced Group List - Begin Code Alteration
//		Original phpBB Code:
		$sql = "SELECT g.group_id, g.group_name, g.group_type, g.group_description, ug.user_pending 
...
*/
		...
		if ( $row = $db->sql_fetchrow($result) )
		{
			do
			{
				...
				if ( $row['user_pending'] )
				{
					$pending_groups[$row['group_id']] = $row;
				}
				else
				{
...
*/
	...
	if ( !empty($group_list_out) || !empty($pending_groups) || !empty($member_groups) )
	{
		//
		// Load and process templates
		//
		...
		if ( !empty($pending_groups) || !empty($member_groups) )
		{
		...
		if ( !empty($member_groups) )
		{
			...
			foreach($member_groups as $k=>$v)
			{
				...
				$template->assign_block_vars('joined.member.grouprow', array(
					'U_GROUP' => append_sid('groupcp.' . $phpEx . '?' . POST_GROUPS_URL . '=' . $k),
					'GROUP_NAME' => $v['group_name'],
					'GROUP_DESC' => $v['group_description'],
		...
		if ( !empty($group_list_out) )
		{
			$template->assign_block_vars('remaining', array() );
			foreach($group_list_out as $k=>$v)
			{
				switch( $v['group_type'] )
				{
					case GROUP_HIDDEN:
