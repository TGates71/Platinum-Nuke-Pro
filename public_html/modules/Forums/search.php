...
//
 // start disable guest search hack
      if ( !( $userdata['session_logged_in'] )) 
      {  
         message_die (GENERAL_MESSAGE, $lang['login_search']); 
      }
//end disable guest search hack
//
// Define initial vars
//
...
/*****************************************************/
	if ( $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'unanswered' || $search_id == 'startedtopics' || $search_keywords != '' || $search_author != '' )
/*****************************************************/
...
/*****************************************************/
	{
		...
		// forums they are allowed to view and include_once them in the search.
		//
...
/*****************************************************/
		...
		// - to include_once or not to include_once
		/*
	...
	if ( $search_results != '' )
	{
		...
		{
/*                        $sql = "SELECT t.*, f.forum_id, f.forum_name, u.username, u.user_color_gc, u.user_id, u2.username as user2, u2.user_color_gc as color2, u2.user_id as id2, p.post_username, p2.post_username AS post_username2, p2.post_time
...
/*****************************************************/
				...
				else
				{
					...
					'POST_DATE' => $post_date,
					'POSTER_NAME' => CheckUsernameColor($searchset[$i]['user_color_gc'],$searchset[$i]['username']),
...
/*****************************************************/
				...
				$topic_author .= ( $searchset[$i]['user_id'] != ANONYMOUS ) ? CheckUsernameColor($searchset[$i]['user_color_gc'], $searchset[$i]['username']) : ( ( $searchset[$i]['post_username'] != '' ) ? $searchset[$i]['post_username'] : $lang['Guest'] );
