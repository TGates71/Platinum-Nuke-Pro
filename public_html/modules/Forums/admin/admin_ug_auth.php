...
//function check_auth_cb($type, $key, $u_access, $is_admin)
{
        $auth_user = 0;
...
/*****************************************************/
        ...
        $db->sql_freeresult($result);
        if (sizeof($group_user))  // Original PNPv1
//      if (count($group_user))   // From EvoX
        {
/*****[BEGIN]******************************************
...
//
        ...
        if ( $user_check != $user_id )
        {
            ...
            $sql = "INSERT INTO " . GROUPS_TABLE . " (group_id, group_name, group_description, group_single_user, group_moderator,user_id)
                    VALUES ('$group_id', '', 'Personal User', '1', '0', '$user_id')"; // added user_id column - sgtmudd
            if ( !($result = $db->sql_query($sql)) )
            {
