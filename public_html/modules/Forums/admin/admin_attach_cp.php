 ...
 *
 *	$Id: admin_attach_cp.php,v 1.23 2004/12/11 16:33:53 acydburn Exp $
 *
 ***************************************************************************/
		...
		for ($i = 0; $i < count($attachments); $i++)
		{
			...
			for ($j = 0; $j < $num_ids; $j++)
			{
				if ($ids[$j]['post_id'] != 0)
				{
					...
					$view_topic = '../../../modules.php?name=Forums&file=viewtopic&' . POST_POST_URL . '=' . $ids[$j]['post_id'] . '#' . $ids[$j]['post_id'];
			...
			$template->assign_block_vars('attachrow', array(
				...
				'U_VIEW_ATTACHMENT' =>'../../../modules.php?name=Forums&file=download&id=' . $attachments[$i]['attach_id'])
//				'U_VIEW_POST' => ($attachments[$i]['post_id'] != 0) ? "../../../modules.php?name=Forums&file=viewtopic& . POST_POST_URL . "=" . $attachments[$i]['post_id'] . "#" . $attachments[$i]['post_id']) : ''
			);
