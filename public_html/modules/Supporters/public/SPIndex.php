...
OpenTable();
echo "<center>";
if(is_admin($admin)) { echo "[ <a href='".$admin_file.".php?op=SPMain'>"._SP_GOTOADMIN."</a> ]\n"; }
if($sp_config['require_user'] == 0 || is_user($user)) { echo "[ <a href='modules.php?name=$module_name&amp;op=SPSubmit'>"._SP_BESUPPORTER."</a> ]\n"; }
echo "</center>";
echo "<br />";
$a = 0;
...
if($numrows > 0) {
  ...
  while(list($site_id, $site_name, $site_url, $site_image, $site_date, $site_description, $site_hits) = $db->sql_fetchrow($result)) {
    ...
    echo "<a href='modules.php?name=$module_name&op=SPGo&site_id=$site_id' target='_blank'><img src='$site_image' border='0' alt='$site_name' title='$site_name' height='$height' width='$width'></a>";
    if(is_admin($admin)) {
