...
$projectresult = $db->sql_query("select project_id, weight from ".$prefix."_nsnwb_projects where weight>='".$project['weight']."'");
while (list($p_project_id, $weight) = $db->sql_fetchrow($projectresult)) {
    $new_weight = $weight - 1;
    $db->sql_query("UPDATE ".$prefix."_nsnwb_projects SET weight='$new_weight' WHERE project_id='$p_project_id'");
    $db->sql_query("OPTIMIZE TABLE ".$prefix."_nsnwb_projects");
}
header("Location: ".$admin_file.".php?op=WBProjectList");
