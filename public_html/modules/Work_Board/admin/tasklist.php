...
echo "<tr><td><img src='$wbimage'></td><td colspan='2' width='100%'><nobr><a href='modules.php?name=$module_name&amp;file=admin&amp;op=WBTaskAdd'>"._WB_TASKADD."</a></nobr></td></tr>\n";
...
OpenTable();
echo "<table width='100%' border='1' cellspacing='0' cellpadding='2'>\n";
echo "<tr><td bgcolor='$bgcolor2' width='100%' colspan='2'><nobr><strong>"._WB_TASKS."</strong></nobr></td>\n";
...
if($taskrows > 0){
    ...
    while (list($task_id, $task_name, $project_id, $priority_id, $status_id) = $db->sql_fetchrow($reviewresult)){
        ...
        echo "<td align='center'><nobr><a href='modules.php?name=$module_name&amp;file=admin&amp;op=WBProjectList'>".$project['project_name']."</a></nobr></td>\n";
        if($taskstatus['status_name'] == ''){ $taskstatus['status_name'] = _WB_NA; }
        echo "<td align=center><nobr><a href='modules.php?name=$module_name&amp;file=admin&amp;op=WBTaskStatusList'>".$taskstatus['status_name']."</a></nobr></td>\n";
        echo "";
        if($taskpriority['priority_name'] == ""){ $taskpriority['priority_name'] = _WB_NA; }
        echo "<td align=center><nobr><a href='modules.php?name=$module_name&amp;file=admin&amp;op=WBTaskPriorityList'>".$taskpriority['priority_name']."</a></nobr></td>\n";
        echo "<td align=center><nobr>[ <a href='modules.php?name=$module_name&amp;file=admin&amp;op=WBTaskEdit&amp;task_id=$task_id'>"._WB_EDIT."</a>";
        echo " | <a href='modules.php?name=$module_name&amp;file=admin&amp;op=WBTaskRemove&amp;task_id=$task_id'>"._WB_DELETE."</a> ]</td></tr>\n";
    }
    echo "<form method='post' action='".$admin_file.".php'>\n";
    echo "<input type='hidden' name='op' value='WBTaskList'>\n";
    ...
    echo "<form method='post' action='".$admin_file.".php'>\n";
    echo "<input type='hidden' name='op' value='WBTaskList'>\n";
    echo "<td align='right' bgcolor='$bgcolor2'><strong>"._WB_SORT.":</strong> <select name='column'>\n";
    ...
    echo "<tr><td colspan='6' width='100%' align='center'>"._WB_NOTASKS."</td></tr>\n";
}
echo "</table>\n";
CloseTable();
