...
if ($adm_info['radminsuper']==1) {
	...
	OpenTable();
	echo "<table width=\"100%\">";
	$resultbans = $db->sql_query("SELECT user_name, reason, id FROM ".$prefix."_banreq WHERE active='No'");
	for ($m=0; $m < $db->sql_numrows($resultbans); $m++){
		list($user_name, $reason, $id) = $db->sql_fetchrow($resultbans);
		echo "  <tr>";
		echo "    <td width='50%'><strong>"._BR_UNAME.":</strong> ".$user_name."</td>";
		echo "    <td width='50%'><a href='modules.php?name=".$module_name."&file=activ&id=".$id."'>"._BR_ACTIVATE."</a></td>";
		echo "  </tr>";
		echo " <tr><td width='100%'><strong>"._BR_BANREASON."</strong><br />".$reason."</td></tr>";
		echo "<tr><td width=100%'><strong>------------------------------------</strong></td></tr>";
	}
	echo "</table>";
	CloseTable();
	// Footer
