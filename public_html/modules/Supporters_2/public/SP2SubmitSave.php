...
if($sp_config['require_user'] == 0 || is_user($user)) {
  ...
  if(!in_array($ext,$limitedext)) {
    include_once("header.php");
    OpenTable();
    echo "<center><strong>"._SP2_BADEXT."</strong></center><br />\n";
    echo "<center>"._GOBACK."</center>\n";
    CloseTable();
    include_once("footer.php");
    die();
  ...
  $result = $db->sql_query("INSERT INTO `".$prefix."_nsnsp_2_sites` values (NULL, '$site_name', '$site_url', '$imgurl', '0', '0', now(), '$site_description', '$user_id', '$user_name', '$user_email', '$user_ip')");
  if(!$result) {
    include_once("header.php");
    OpenTable();
    ...
    $msg = "$sitename "._SP2_ADDED."\n\n";
    $msg .= _SP2_NAME.": ".stripslashes($site_name)."\n";
    $msg .= _SP2_URL.": ".stripslashes($site_url)."\n";
    $msg .= _SP2_IMAGE.": ".stripslashes($imgurl)."\n";
    $msg .= _SP2_DESCRIPTION.": ".stripslashes($site_description)."\n";
    $msg .= _SP2_USERID.": $user_id\n";
    $msg .= _SP2_USERNAME.": ".stripslashes($user_name)."\n";
    $msg .= _SP2_USEREMAIL.": ".stripslashes($user_email)."\n";
    $msg .= _SP2_USERIP.": $user_ip\n";
    $to = $adminmail;
    $subject = "$sitename - "._SP2_ADDED;
