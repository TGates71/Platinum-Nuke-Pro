...
/*                                                                           */
/* USE THIS FILE IF YOU CONVERTED FROM EVO XTREME 209D TO PLATINUM NUKE PRO  */
/* RENAME index.php TO index_orig.php                                        */
/* THEN RENAME index_evox.php to index.php                                   */
/*****************************************************************************/
...

{
	...
	$redirect = ( $folder ) ? "folder=$folder" : 'folder=inbox';

	$redirect .= ( $mode && $privmsg_id ) ? "&mode=$mode&" . POST_POST_URL . "=$privmsg_id" : '';
...

	{
		...
		header($header_location . append_sid("login.$phpEx?redirect=privmsg.$phpEx&$redirect", true));
...

	{
		...
		$message = $lang['Buddy_added'] . '<br /><br />' . sprintf($lang['Click_return_message'], '<a href="' . append_sid ("privmsg.$phpEx?$redirect") . '">', '</a>');
...

	{
		...
		$l_click_return = ( $mode && $privmsg_id ) ? $lang['Click_return_message'] : $lang['Click_return_inbox'];

		$message = $lang['Buddy_removed'] . '<br /><br />' . sprintf($l_click_return, '<a href="' . append_sid("privmsg.$phpEx?$redirect") . '">', '</a>');
	...
	$template->assign_vars(array(
		...
		'META' => '<meta http-equiv="refresh" content="3;url=' . append_sid("privmsg.$phpEx?$redirect") . '">')
...
/*****************************************************/
        ...
        $user_id = $privmsg['user_id_2'];

        $buddy_id = $privmsg['user_id_1'];



        $redirect = ( isset($privmsg_id) ) ? POST_POST_URL . "=$privmsg_id" : '';

        $redirect .= ( isset($mode) && isset($privmsg_id) ) ? "&amp;mode=$mode" : '';

        $redirect .= ( isset($folder) ) ? "&amp;folder=$folder" : '&amp;folder=inbox';
...

        {
            ...
            message_die(GENERAL_ERROR, 'Could not query buddy information', '', __LINE__, __FILE__, $sql);
...
/*****************************************************/
...

        {
                ...
                $sql = "INSERT $sql_priority INTO " . PRIVMSGS_TABLE . " (privmsgs_type, privmsgs_subject, privmsgs_from_userid, privmsgs_to_userid, privmsgs_date, privmsgs_ip, privmsgs_enable_html, privmsgs_enable_bbcode, privmsgs_enable_smilies, privmsgs_attach_sig)
                        ...
                        VALUES (" . PRIVMSGS_SENT_MAIL . ", '" . str_replace("\'", "''", htmlspecialchars(addslashes($privmsg['privmsgs_subject']))) . "', " . $privmsg['privmsgs_from_userid'] . ", " . $privmsg['privmsgs_to_userid'] . ", " . $privmsg['privmsgs_date'] . ", '" . $privmsg['privmsgs_ip'] . "', " . $privmsg['privmsgs_enable_html'] . ", " . $privmsg['privmsgs_enable_bbcode'] . ", " . $privmsg['privmsgs_enable_smilies'] . ", " .  $privmsg['privmsgs_attach_sig'] . ")";
                ...
                $sql = "INSERT $sql_priority INTO " . PRIVMSGS_TEXT_TABLE . " (privmsgs_text_id, privmsgs_bbcode_uid, privmsgs_text)
                        ...
                        VALUES ('$privmsg_sent_id', '" . $privmsg['privmsgs_bbcode_uid'] . "', '" . htmlspecialchars(str_replace("\'", "''", addslashes($privmsg['privmsgs_text']))) . "')";
...
/*****************************************************/
...

        {
                ...
                $buddy_img = ( $buddy_action == 'remove' ) ? '<a href="' . append_sid("privmsg.$phpEx?$redirect") . '"><img src="' . $images['icon_buddy_remove'] . '" alt="' . $lang['Remove_buddy_list'] . '" border="0"></a>' : '<a href="' . append_sid("privmsg.$phpEx?$redirect") . '"><img src="' . $images['icon_buddy'] . '" alt="' . $lang['Add_buddy_list'] . '" border="0"></a>';

                $buddy = ( $buddy_action == 'remove' ) ? '<a href="' . append_sid("privmsg.$phpEx?$redirect") . '">' . $lang['Remove_buddy_list'] . '</a>' : '<a href="' . append_sid("privmsg.$phpEx?$redirect") . '">' . $lang['Add_buddy_list'] . '</a>';
...
/*****************************************************/
	...
	if ( $privmsg['user_session_time_1'] >= (time()-3600) )

	{
...

		{
			...
			$online_status = '&nbsp;(<strong><i><a href="' . append_sid("viewonline.$phpEx") . '" title="' . sprintf($lang['is_hidden'], $username_from) . '"' . $hidden_color . '>' . $lang['Hidden'] . '</a></i></strong>)';
...

		{
			...
			$online_status = '&nbsp;(<strong><span title="' . sprintf($lang['is_offline'], $username_from) . '"' . $offline_color . '>' . $lang['Offline'] . '</span></strong>)';
	...
	if ( $privmsg['user_session_time_2'] >= (time()-3600) )

	{
...

		{
			...
			$online_status_2 = '&nbsp;(<strong><a href="' . append_sid("viewonline.$phpEx") . '" title="' . sprintf($lang['is_online'], $tusername_to) . '"' . $online_color . '>' . $lang['Online'] . '</a></strong>)';
...

		{
			...
			$online_status_2 = '&nbsp;(<strong><i><a href="' . append_sid("viewonline.$phpEx") . '" title="' . sprintf($lang['is_hidden'], $username_to) . '"' . $hidden_color . '>' . $lang['Hidden'] . '</a></i></strong>)';
...

		{
			...
			$online_status_2 = '&nbsp;(<strong><span title="' . sprintf($lang['is_offline'], $username_to) . '"' . $offline_color . '>' . $lang['Offline'] . '</span></strong>)';
...
/*****************************************************/
    ...
    include_once("includes/ropm_quick_reply.php");
...
/*		// session id check
	...
	/* End Admin PM Mod */
                        ...
                        $sql_info = "INSERT INTO " . PRIVMSGS_TABLE . " (privmsgs_type, privmsgs_subject, privmsgs_from_userid, privmsgs_to_userid, privmsgs_date, privmsgs_ip, privmsgs_enable_html, privmsgs_enable_bbcode, privmsgs_enable_smilies, privmsgs_attach_sig)
                                ...
                                VALUES (" . PRIVMSGS_NEW_MAIL . ", '" . htmlspecialchars(str_replace("\'", "''", $privmsg_subject)) . "', " . $userdata['user_id'] . ", " . $to_userdata['user_id'] . ", $msg_time, '$user_ip', '$html_on', '$bbcode_on', '$smilies_on', '$attach_sig')";
...

                {
                        ...
                        $sql_info = "UPDATE " . PRIVMSGS_TABLE . "
                                ...
                                SET privmsgs_type = " . PRIVMSGS_NEW_MAIL . ", privmsgs_subject = '" . htmlspecialchars(str_replace("\'", "''", $privmsg_subject)) . "', privmsgs_from_userid = " . $userdata['user_id'] . ", privmsgs_to_userid = " . $to_userdata['user_id'] . ", privmsgs_date = '$msg_time', privmsgs_ip = '$user_ip', privmsgs_enable_html = '$html_on', privmsgs_enable_bbcode = '$bbcode_on', privmsgs_enable_smilies = '$smilies_on', privmsgs_attach_sig = '$attach_sig'
...

                {
                        ...
                        $sql = "INSERT INTO " . PRIVMSGS_TEXT_TABLE . " (privmsgs_text_id, privmsgs_bbcode_uid, privmsgs_text)
                                ...
                                VALUES ('$privmsg_sent_id', '" . $bbcode_uid . "', '" . htmlspecialchars(str_replace("\'", "''", $privmsg_message)) . "')";
...

                {
                        ...
                        $sql = "UPDATE " . PRIVMSGS_TEXT_TABLE . "
                                ...
                                SET privmsgs_text = '" . htmlspecialchars(str_replace("\'", "''", $privmsg_message)) . "', privmsgs_bbcode_uid = '$bbcode_uid'
...
include_once('includes/mini_buddylist.'.$phpEx);
