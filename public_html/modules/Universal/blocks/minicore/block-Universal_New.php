...
$result = $db->sql_query("SELECT id, title FROM ".$prefix."_".$mainprefix."_items where language = '$unilang' AND active = '1' ORDER BY date DESC LIMIT 0,5");
while(list($eid, $title) = $db->sql_fetchrow($result)) {
	$title = stripslashes($title);
    $uninewcontent .= "<strong><big>&middot;</big></strong>&nbsp;<a href=\"modules.php?name=$modulename&amp;op=ViewItems&amp;vid=$eid\">$title</a><br>";
}
