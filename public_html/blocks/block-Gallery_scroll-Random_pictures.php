...
while ($row = $db->sql_fetchrow($result)) {
   ...
   $thumb_title = $row['filename'];
    ...
    $content .= '<a href="' . $CPG_M_URL . '&amp;file=displayimage&amp;album='.$row['aid'].'&amp;pid=' . $row["pid"] . '"><img src="' .get_pic_url($row, 'thumb') . '" border="0" alt="' . $thumb_title . '" title="' . $thumb_title . '"><br />' . truncate_stringblocks($thumb_title,$maxlength) . '</a><br />';
} 
