...
$supporterrt = $db->sql_fetchrow($db->sql_query("SELECT * from ".$prefix."_nsnsp_2_config"));
...
$result = $db->sql_query("SELECT site_id, site_name, site_image FROM $prefix"._nsnsp_2_sites." WHERE site_status>'0' ORDER BY site_name");
while (list($site_id, $site_name, $site_image) = $db->sql_fetchrow($result)) {
    $content .= "&nbsp;<a href='modules.php?name=Supporters_2&op=SP2Go&site_id=$site_id' target='_blank'><img src='$site_image' height='31' width='88' title='$site_name' border='0'></a>&nbsp;\n";
}
$content .="</marquee><br /><br />\n";
if ($supporterrt['require_user'] == 0 || is_user($user)) { $content .= "[ <a href='modules.php?name=Supporters_2&amp;op=SP2Submit'>"._SP2_BESUPPORTER."</a> ]<br />\n"; }
