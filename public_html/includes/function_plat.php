...
function check_download_token($token) {
  ...
  if (isset($download_token[serialize(md5($token))])) {
    return true;
  }
...
function get_file_extension($file_name) {
  preg_match("/(.+)\.(.+)/", get_basefile($file_name), $regs);
...
function get_user_os() {
  global $global_info, $HTTP_USER_AGENT, $_SERVER;
...
if ( !function_exists('mime_content_type') ){
   function mime_content_type($f){
       exec(trim('file -bi '.escapeshellarg($f)), $output);
		return $output[0];
   }
}
...
function check_ban()
{
  global $user_info, $site_sess, $config, $lang, $site_db, $_GET;
  ...
  if ($user_info['user_level'] == ADMIN)
  {
    if (!isset($_GET['bantest'])) return false;
    $return = true;
    foreach ($types as $key)
    {
      if (isset($_GET[$key]) && $$key = $_GET[$key]) $return = false;
      else $$key = "";
    }
