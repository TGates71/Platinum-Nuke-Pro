 ...
 * @subpackage Core
 * @version $Id: mysqli.php 3780 2011-12-26 07:00:08Z palbin $
		...
		if (mysqli_connect_error()) {
			...
			*/
			...
			if (version_compare($this->db_version, '40100', '>=')) {
				...
				$this->dbVersionCompare = '<p>&nbsp;</p><p class="d1"><strong>We are sorry but the the MySQL version (' . $this->db_version . ') you are attempting to use is to old and is not supported by Platinum Nuke Pro'
	...
	public function sql_query($query = '', $transaction = FALSE) {
		...
		*/
		...
		if ($this->loglevel == 2 || $this->loglevel == 4) {
			...
			$fplog = fopen(NUKE_BASE_DIR . 'logs/dblog', 'a');
			fwrite($fplog, $logvar . "\n");
			fclose($fplog);
		}
