...
class SQLCtrl {
	...
	function backup($tables, $filename, $structure = true, $data = true, $drop = true, $compress = true) {
        global $dbname;
			...
			$str .= "INSERT INTO ".$table." ".$fields." VALUES (";
				for($j=0; $j<$fieldcount;$j++) {
					if ($j>0) $str .= ", ";
						...
						elseif($row[$j] != "") { $str .= "'".addcslashes($row[$j], $crlf."'")."'"; }
						else { $str .= "''"; }
				}
            		$str .= ");".$crlf;
		}
	...
	function get_table_struct($table, $crlf, $drop) {
	global $db;
	$schema_create = '';
	if ($drop) {
	...
	if (empty($file['tmp_name']) || empty($file['name'])) { $error = _BACKUP_RESTORE_NOFILE; return false; }
